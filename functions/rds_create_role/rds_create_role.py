...
def lambda_handler(event, context):
  ...
  if cfnMsgParams['ResourceStatus'] == 'CREATE_COMPLETE':
    ...
    if cfnMsgParams['ResourceType'] == 'AWS::RDS::DBInstance':
      ...
      db_instance_identifier = cfnMsgParams['PhysicalResourceId']

      # Snag Database ID, Master Username & Password from CloudFormation notification message
      db_instance_identifier = cfnMsgParams['PhysicalResourceId']
      ...
      try:
        ...
        raise Exception('Could not query AWS for DBInstance Endpoint Address: ' + e['Error']['Message'])
